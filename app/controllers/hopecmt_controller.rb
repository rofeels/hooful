#encoding: UTF-8
class HopecmtController < ApplicationController
  respond_to :json
  TALK_LIMIT = 5
  def index
	if params[:firstTalk]
		@ehootalk = Hopecmt.where(:rCode => params[:rCode], :_id.lt =>params[:firstTalk]).desc(:created_at)
	else
		@ehootalk = Hopecmt.where(:rCode => params[:rCode]).desc(:created_at)
	end
	@hootalk = []
  ...
  def create
	Hopecmt.create(:rCode => params[:rCode],:rMsg => params[:rMsg],:rUserid => params[:rUserid])
  Hope.incCmt(params[:rCode]) if params[:rCode] != ""
	...
	if params[:lastTalk]
		@ehootalk = Hopecmt.where(:rCode => params[:rCode], :_id.gt =>params[:lastTalk])
	else
		@ehootalk = Hopecmt.where(:rCode => params[:rCode]).desc(:created_at).limit(TALK_LIMIT).reverse
	end
	@hootalk = []
	@ehootalk.each do |hootalk|
		mtmp = Hash.new
