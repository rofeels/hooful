...
class GroupController < ApplicationController
  ...
  def index
    ...
    if params[:category]
		...
		if params[:lastGroup]
			@egroups = TicketSold.where(:mUserid=>params[:mUserid],:tState.in => [1,2,3,4,5,10],:_id.lt =>params[:lastGroup]).desc(:created_at)
		else
			@egroups = TicketSold.where(:mUserid => params[:mUserid],:tState.in => [1,2,3,4,5,10]).desc(:created_at)
		end
		...
		@egroups.limit(GROUP_LIMIT).each do |group|
			...
			if !minfo.mDateS.blank?
				gtmp["gDate"] = minfo.mDateS.strftime("%m.%d")
			else
				gtmp["gDate"] = "미정"
			end

			  ghinfo = User.info(minfo.mHost)
			  ghtmp = Hash.new
			  ghtmp[:name] = ghinfo.name
			  ghtmp[:picture] = ghinfo.picture
			  ghtmp[:userid] = ghinfo.userid
			  ...
			  ghtmp[:members] = ghinfo.members
			gtmp["gHost"] = ghtmp
			gtmp["mCode"] = minfo.mCode
			gtmp["mMale"] = minfo.mMale
			gtmp["mFemale"] = minfo.mFemale
			gtmp["uMale"] = minfo.uMale
			gtmp["uFemale"] = minfo.uFemale

			gtmp["gTitle"] = minfo.mTitle
			@groups << gtmp
		end
